(function (factory) {
  ...
  var drawerkolo1 = /*#__PURE__*/function () {
    function drawerkolo1(options) {
      _classCallCheck(this, drawerkolo1);

      this.element = document.getElementById('kolo1');
      this.delegateRoot = new Delegate(document.documentElement);
      this.options = options;
      this.isOpen = false;
    ...
    _createClass(drawerkolo1, [{
      ...
      key: "_attachListeners",
      value: function _attachListeners() {
        this.delegateRoot.on('click', '[aria-controls="kolo1"][data-action="open-drawer"]', this.open.bind(this));
        this.delegateRoot.on('click', "#kolo1 [data-action=\"close-drawer\"]", this.close.bind(this));
      ...
      value: function open(event) {
        ...
        this.isOpen = true;
        document.querySelector('[aria-controls="kolo1"]').setAttribute('aria-expanded', 'true');
        document.getElementById('kolo1').setAttribute('aria-hidden', 'false');
        document.body.classList.add('no-mobile-scroll');
      }
    }, {
      key: "close",
      value: function close(event) {
        ...
        this.isOpen = false;
        document.querySelector('[aria-controls="kolo1"]').setAttribute('aria-expanded', 'false');
        document.getElementById('kolo1').setAttribute('aria-hidden', 'true');
        document.body.classList.remove('no-mobile-scroll');
      }
    }, {
      key: "_computeDrawerHeight",
      value: function _computeDrawerHeight() {
        document.getElementById('kolo1').querySelector('.collection-drawer').style.maxHeight = "".concat(window.innerHeight, "px");
      }
    }, {
      key: "_detectOutsideClick",
      value: function _detectOutsideClick(event) {
  ...
  */
  ...
  var CollectionSection = /*#__PURE__*/function () {
    function CollectionSection(element) {
      ...
      this.drawerkolo1 = new drawerkolo1(this.options);
...
queries = [fetch("".concat(url, "?section_id=predictive-search&q=").concat(productQuery, "&resources[options][fields]=body,product_type,title,variants.barcode,variants.sku,variants.title,vendor&resources[limit]=8&resources[limit_scope]=each"), queryOptions)];          Promise.all(queries).then(function (responses) {
...
const element = document.getElementById('glglg');

const observer = new MutationObserver(function(mutations) {
  mutations.forEach(function(mutation) {
    if (mutation.attributeName === 'aria-hidden') {
      if (miniCart.getAttribute('aria-hidden') === 'false') {
        element.classList.add('csd');
      } else {
        element.classList.remove('csd');
      }
    }
  });
});
